...
namespace SharpMap
{
    ...
    public class Map : IDisposable
    {
        ...
        /// <summary>
        /// 针对DEM数据切片（16位整形）,输入数据为单波段16位tif
        /// </summary>
        /// <returns></returns>
        public short[] GetDEMTileDataset(double tileWest, double tileNorth, double levelPerDegree)
        {
            short[] demtileds = new Int16[Size.Width * Size.Height];

            demtileds = FillDEMTile(demtileds, tileWest, tileNorth, levelPerDegree);
            return demtileds;
        }
        ...
        /// </summary>
        /// <returns></returns>
        private short[] FillDEMTile(short[] demtileds, double tileWest, double tileNorth, double levelPerDegree)
        {
            ...
            {
                if (layer.Enabled && layer.MaxVisible >= Zoom && layer.MinVisible < Zoom)
                    demtileds = layer.FillDEMTile(demtileds, tileWest, tileNorth, levelPerDegree);
            }
        ...
        /// <summary>
        /// 徐辉添加，产生缩略图的，对Graphics 不做背景色填充豫处理，这样，两幅影像镶嵌就可以在一起
        /// </summary>
        /// <param name="g"></param>
        public void RenderOverViewMap(Graphics g)
        {
            if (g == null)
                throw new ArgumentNullException("g", "Cannot render map with null graphics object!");
