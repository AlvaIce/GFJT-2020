...
namespace QRST_DI_MS_Desktop
{
    public partial class frm_MSConsole2 : DevExpress.XtraEditors.XtraForm
    {
		...
		public frm_MSConsole2()
        {
            ...
            ribbonPage17.Visible = false;
			this.WindowState = FormWindowState.Maximized;

            //注册绑定子系统功能UI
            InitializeMSUserInterfase();
           InputDataService.currentUser = TheUniversal.currentUser;
        ...
        private void InitializeMSUserInterfase()
        {
			...
			itemList.Add(baritem_sysDataMaintain);
            //itemList.Add(barStaticItem4);
            //20160409注释元数据管理子系统 For 四川
            //itemList.Add(barStaticItem5);
			itemList.Add(barStaticItem6);
            itemList.Add(barStaticItem7);
            itemList.Add(barStaticItem3);
            //实例化全局变量
            IDbBaseUtilities mySqlBaseUtilities = Constant.IdbServerUtilities;
            ...
            for (int i = 0 ; i < subDbArr.Rows.Count ; i++)
            {
                ...
                string relativeStr = subDbArr.Rows[i]["ConnectStr"].ToString();
            ...
            //rucmetadatadefiner.GetTreeDir += new rucMetadataDefiner.MyEventHandler(mucmetadatadefiner.left.GetTreeDir);
            ...
            //子系统任务监控
            mucTaskOrderMonitor muctaskordermonitor = new mucTaskOrderMonitor();
            rucTaskOrderManager ructaskorderManager = new rucTaskOrderManager(muctaskordermonitor);
            AddNewMSUI(ructaskorderManager, muctaskordermonitor, nbi_TaskSubMonitor);

            //子系统计算服务
            mucComputeService muccomputeservice = new mucComputeService();
            rucComputeService ruccomputeservice = new rucComputeService(muccomputeservice);
            AddNewMSUI(ruccomputeservice, muccomputeservice, nbi_Compute);
            ...
            //虚拟文件夹管理2017-11-28 yb
            //mucVirtualDirManager mucvirtualdirmgr = new mucVirtualDirManager();
            //rucVirtualDirManager rucvirtualdirmgr = new rucVirtualDirManager(mucvirtualdirmgr);
            //AddNewMSUI(rucvirtualdirmgr, mucvirtualdirmgr, nbi_VirtualDirManager);

            //数据导入
            mucDataImport mucdataImport = new mucDataImport();
            rucDataImport rucdataImport = new rucDataImport(mucdataImport);
            ...
            TheUniversal.MainForm = this;
            TheUniversal.RucDataImport = rucdataImport;
            this.WindowState = FormWindowState.Maximized;
        }
        /// <summary>
        /// 创建新的UI
        ...
        public void setNaviItemSelected(string strIn)
        {
            ...
            //{
                ...
                {
                    if (nbc_sysNavi.Items[i].Caption.ToString() == strIn)
                    {
                        nbc_sysNavi.SelectedLink =nbc_sysNavi.Items[i].Links[0];
                        break;
                    }
                }
            //}
        ...
        public void systemselect_ItemClick(object sender, ItemClickEventArgs e)
        {
            systemselect_ItemClick(sender, e, "");
        }

        public void systemselect_ItemClick(object sender, ItemClickEventArgs e, string tabpagename)
        {
            ...
            systemSelect(e.Item.Caption, tabpagename);
        }
		...
		private void systemSelect(string name,string tabpagename="")
		{
			...
			foreach (NavBarGroup item in nbc_sysNavi.Groups)
			{
                if (name.Contains(item.Caption))
                {
                    ...
                    MSUserInterface firstmsui = null;
                    //模块
                    foreach (MSUserInterface msui in MSUserInterface.listMSUI)
                    {
                        if (sysmodel[msui.uiSysNavBarItem] == item)
                        {
                            //20160409注释订单定制和任务定制 For 四川
                            if (msui.uiSysNavBarItem.Caption == "订单定制" || msui.uiSysNavBarItem.Caption == "任务定制" || msui.uiSysNavBarItem.Caption == "数据备份恢复")
                            {
                                msui.uiRibbonPage.Visible = false;
                            }
                            else
                            ...
                            if (tabpagename != "" && msui.uiRibbonPage.Text == tabpagename)
                            {
                                ribbon.SelectedPage = msui.uiRibbonPage;
                                isselected = true;
                            }
                    ...
                    if (!isselected && firstmsui != null)
                    {
                        ribbon.SelectedPage = firstmsui.uiRibbonPage;
                        isselected = true;
                    }
        ...
        private void frm_MSConsole2_FormClosed(object sender, FormClosedEventArgs e)
        {
            Process.GetCurrentProcess().Kill();
        }

        private void frm_MSConsole2_FormClosing(object sender, FormClosingEventArgs e)
        {
            NoticeMessagerForFileBackup.SystemExited = true;
        }
    }
}
