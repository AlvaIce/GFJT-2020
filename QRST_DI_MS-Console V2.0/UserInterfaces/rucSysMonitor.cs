...
namespace QRST_DI_MS_Desktop.UserInterfaces
{
    public partial class rucSysMonitor :RibbonPageBaseUC
    {
        ...
        private void barButtonItemQuery_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            {
                startTime = ((DateTime)barEditStartTime.EditValue).ToString("yyyy-MM-dd HH:mm:ss");

                if (!string.IsNullOrEmpty(sb.ToString()))
                {
                    sb.Append(" and ");
                }
                sb.AppendFormat("(日志时间 between '{0}' ", startTime);
            }
            string endTime = "5999-12-30 23:59:59";
            if (barEditEndTime.EditValue != null)
            {
                endTime=((DateTime)barEditEndTime.EditValue).ToString("yyyy-MM-dd HH:mm:ss");
                if (!string.IsNullOrEmpty(sb.ToString()))
                {
                    sb.Append(" and ");
                }
                sb.AppendFormat("'{0}')",endTime);
            }
