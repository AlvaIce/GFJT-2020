...
namespace SharpMap.Layers
{
    ...
    public class GdalRasterLayer : Layer, ICanQueryLayer, IDisposable
    {
        ...
        public GdalRasterLayer(string strLayerName, string imageFilename, object thislock):this(strLayerName,imageFilename)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public short[] FillDEMTile(short[] demtileds, Map map)
        {
            if (disposed)
                throw (new ApplicationException("Error: An attempt was made to render a disposed layer"));

            //徐辉添加：先进行投影转换，把影像坐标转换为地图坐标。由于Worldwind是GCS坐标，所以需要先把影像投影坐标转换为GCS坐标
            ReprojectToMap(map);
            demtileds = GetDEMTile(_gdalDataset, map.Size, demtileds, map.Envelope, null, map);
            return demtileds;
        }
        ...
        protected short[] GetDEMTile(Dataset dataset, Size size, short[] demtileds,
