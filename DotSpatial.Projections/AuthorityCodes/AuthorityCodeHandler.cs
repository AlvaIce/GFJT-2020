...
namespace DotSpatial.Projections.AuthorityCodes
{
    ...
    public sealed class AuthorityCodeHandler
    {
        ...
        private void ReadDefault()
        {
            ...
            try
            {
                object obj = Properties.Resources.ResourceManager.GetObject("AuthorityCodeToProj4_txt");
                using (StringReader sr = new StringReader(obj.ToString()))
                {
                    char[] seperator = new[] { '\t' };
                    ...
                    while (line != null)
                    {
                        ...
                        {
                            if (parts.Length == 2)
                                Add(parts[0], string.Empty, parts[1], true);
                            else
                                Add(parts[0], parts[1], parts[2], true);
                        }
                        line = sr.ReadLine();
                    }
                }
                //ReadFromStream(s, true);
            }
            catch{
            ...
            //using (Stream s =Assembly.GetCallingAssembly().GetManifestResourceStream(
