...
namespace TilesImport
{
    public class TileStore
    {
        ...
        private FormTileImport form;
        public TileStore(FormTileImport form)
        {
            TaskID.Clear();
            tasks.Clear();
        ...
        public void TilesImport(object obj)
        {
            ...
            form.ShowmessageLabel2("正在建立切片索引……");
            form.ShowmessageLabel3("");
            form.ShowmessageLabel4("");
            form.ShowmessageLabel5("");
            form.ShowmessageLabel6("");
            form.ShowmessageLabel7("");
            form.ShowmessageLabel8("");
            form.ShowmessageLabel9("");
            form.ShowmessageLabel10("");
            if (isClassify)
            {
        ...
        {
            string importUrl = form.textBox1.Text;
                ...
                foreach (var item in files)
                {
                    ...
                    if (item.Substring(item.LastIndexOf("\\") + 1).Substring(0, 2) == "CS")
                    {
                        if (item.Substring(item.LastIndexOf(".") + 1) == "txt")
                        {
                            ...
                            string str = ConfigurationSettings.AppSettings["ConnectionStringSampleMySql"];
                ...
                foreach (string tilesPath in tilesPaths)
                {
                    ...
                    foreach (FileInfo fi in fileinfos)
                    {
                        if (fi.Name.ToUpper().EndsWith(".TIF") || fi.Name.ToUpper().EndsWith(".JPG") || fi.Name.ToUpper().EndsWith(".PNG"))
                        {
                            ...
                            desPath = desPath.Substring(0, desPath.Length - 4) + fi.Name.Substring(fi.Name.Length - 4, 4);      //zsm
                            if (ip != "-1" && ip !="0")
                            {
                    ...
                    form.ShowmessageLabel2("分配任务已完成，共分" + taskNum + "个任务");
                    ...
                    if (taskNum <= MAX_TASK_NUM)
                    {
                        ...
                        form.ShowmessageLabel7("任务已全部启动，共" + taskNum + "个任务正在执行！请耐心等待……");
                    }
                    else
                    {
                        ...
                        form.ShowmessageLabel7(MAX_TASK_NUM + "个任务正在执行,还有" + (taskNum - MAX_TASK_NUM) + "个任务未启动，请耐心等待……");
                        //下一个未启动的任务的索引
                        ...
                        while (!isComplement)
                        {
                            ...
                            if (hasComplement)
                            {
                                ...
                                taskRun[i].Start();
                                form.ShowmessageLabel7(String.Format("已完成{0}个任务，{1}个任务正在执行,还有{2}个任务未启动，请耐心等待……", (runIndex - MAX_TASK_NUM), MAX_TASK_NUM, (taskNum - runIndex)));
                                //若所有任务已启动，则取消监视
                                if (runIndex == taskNum)
                                    isComplement = true;
                            }
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                finally
                {
                    if (i % 20 == 0 && i != 0 && (eachTaskNum[taskid] - i + 1) > 0)
                    {
                        ...
                        {
                            case 0:
                                form.ShowmessageLabel2("任务" + (taskid + 1) + "已完成" + i + "条任务，还有" + (eachTaskNum[taskid] - i + 1) + "条未完成！"); break;
                            case 1:
                                form.ShowmessageLabel3("任务" + (taskid + 1) + "已完成" + i + "条任务，还有" + (eachTaskNum[taskid] - i + 1) + "条未完成！"); break;
                            case 2:
                                form.ShowmessageLabel4("任务" + (taskid + 1) + "已完成" + i + "条任务，还有" + (eachTaskNum[taskid] - i + 1) + "条未完成！"); break;
                            case 3:
                                form.ShowmessageLabel5("任务" + (taskid + 1) + "已完成" + i + "条任务，还有" + (eachTaskNum[taskid] - i + 1) + "条未完成！"); break;
                            case 4:
                                form.ShowmessageLabel6("任务" + (taskid + 1) + "已完成" + i + "条任务，还有" + (eachTaskNum[taskid] - i + 1) + "条未完成！"); break;
                            case 5:
                                form.ShowmessageLabel8("任务" + (taskid + 1) + "已完成" + i + "条任务，还有" + (eachTaskNum[taskid] - i + 1) + "条未完成！"); break;
                            case 6:
                                form.ShowmessageLabel9("任务" + (taskid + 1) + "已完成" + i + "条任务，还有" + (eachTaskNum[taskid] - i + 1) + "条未完成！"); break;
                            case 7:
                                form.ShowmessageLabel10("任务" + (taskid + 1) + "已完成" + i + "条任务，还有" + (eachTaskNum[taskid] - i + 1) + "条未完成！"); break;
                        }
                    }
                }
            }
        ...
        private void BuildTilesIndex(string[] ips, string[] filenames, List<string> listFTNs)
        {
            ...
            UpdateTilesIndex(tilefileNamesPerIP, TileIndexUpdateType.Up);
            //更新失败的切片
            UpdateFailedTilesIndex(listFTNs, TileIndexUpdateType.Up);
        ...
        private void UpdateTilesIndexTask(List<string> namelist)
        {
            try
            {
                ...
                namelist.RemoveAt(0);
                site.TCPService.UpdateTileIndex(TileIndexUpdateType.Up, namelist);
            }
            catch (Exception ex)
            {
                System.Windows.Forms.MessageBox.Show(ex.Message);
