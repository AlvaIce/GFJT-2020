...
{
    public class ITStoreTiles:TaskClass
    {
        ...
        public override void Process()
        {
            ...
            ProcessStoreTiles(orderWorkspace, out filenames, out ips, out failedTiles);
            ...
            this.ParentOrder.Logs.Add(string.Format("正在建立切片索引。"));
        ...
        private bool ProcessStoreTiles( string orderWorkspace, out string[] filenames, out string[] ips, out string[] failedTiles)
        {
            ...
            bool rtn = StoreTiles(tilePaths,  out filenames, out ips, out failedTiles);
                    ...
                    foreach (FileInfo fi in di_data.GetFiles("*.*", SearchOption.AllDirectories))
                    {
                        if (fi.Name.ToUpper().EndsWith(".TIF") || fi.Name.ToUpper().EndsWith(".JPG") || fi.Name.ToUpper().EndsWith(".PNG"))
                        {
                            ...
                            try
                            {
                                ...
                                destPath = destPath.Substring(0, destPath.Length - 4) + fi.Name.Substring(fi.Name.Length - 4, 4);      //zsm
