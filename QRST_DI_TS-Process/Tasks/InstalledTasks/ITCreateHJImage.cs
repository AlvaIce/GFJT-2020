...
{
    public class ITCreateHJImage : TaskClass
    {
        /// <summary>
        /// 任务名,定义唯一标识，不可动态修改
        /// </summary>
        ...
        public override void Process()
        {
            ...
            string destDir = StorageBasePath.StorePath_SourceProject(sourceFilePath);
            ...
            try
            {
                ...
                if (File.Exists(imagePath))
                {
                    ...
                    //向文件服务器发送快视图
                    string destPath = string.Format(@"\\{0}\{1}", Constant.FileServerIP, StaticStrings.Thumbnail);
                    this.ParentOrder.Logs.Add("正在发送快视图！");
                    if (Directory.Exists(destPath))
                    {
                        destPath = string.Format(@"{0}\{1}.JPG", destPath, this.ParentOrder.OrderParams[0]);
                        File.Copy(imagePath, destPath, true);
                        this.ParentOrder.Logs.Add("成功发送快视图！");
                    }
                    else
                    {
                        this.ParentOrder.Logs.Add("没能找到快视图文件夹！");
                    }
                }
                else
                {
                    this.ParentOrder.Logs.Add("没有找到快视图！");
                }
