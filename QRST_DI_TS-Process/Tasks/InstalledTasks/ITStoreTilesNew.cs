...
{
    public class ITStoreTilesNew : TaskClass
    {
        //判断任务是否已全部启动
        ...
        {
            get { return "ITStoreTilesNew"; }
            set { }
        }

        public override void Process()
        {
            string tilesPath = this.ProcessArgu[0];  //切片路径
            tasks.Clear();
            ...
            //调用段龙方写的切片入库方法
            TilesImport(tilesPath,this.ParentOrder.Logs);
        }

        //private void TilesImport(string tilePath, OrderLog log)
        //{
        //    log.Add("...");
        //}
        /// <summary>
        ...
        //public void TilesImport(string tilesPath, OrderLog log)
        //{
        ...
        //                        if (ip != "-1" && TServerSiteManager.optimalStorageSites.Contains(TServerSiteManager.getSiteFromSiteIP(ip)))
        //                        {
        ...
        //                            //else
        //                            //{
        //                            //    string jgwSourcePath = Path.Combine(Path.GetDirectoryName(fi.FullName), Path.GetFileNameWithoutExtension(fi.FullName) + ".jgw");
