...
namespace QRST_DI_MS_Console.UserInterfaces
{
    public partial class rucDeviceManage : RibbonPageBaseUC
    {
        ...
        string ftTempname;
        NormalPagingQuery normalPagingQuery = new NormalPagingQuery();
        string sql;
        ...
        private void barButtonItem4_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            for (int i = 0; i < zdstring.Count; i++)
            {
                ...
                {
                    string str = string.Format("delete from {0} where id='{1}'", normalPagingQuery.tableName, zdstring[i]);
                    cmd.CommandText = str;
                }
                else
                {
                    string str = string.Format("delete from {0} where id='{1}'", mucdevicemaint.tableName, zdstring[i]);
                    cmd.CommandText = str;
                }
                try
                {
