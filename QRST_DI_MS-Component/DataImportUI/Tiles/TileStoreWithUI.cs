...
namespace TilesImport
{
    public class TileStoreWithUI
    {
        ...
        private static IDbBaseUtilities baseUtilities = Constant.IdbServerUtilities;
        public TileStoreWithUI(UCTileImport mainui)
        {
            TaskID.Clear();
            tasks.Clear();
        ...
        public void TilesImport(object obj)
        {
            ...
            Dictionary<string, List<string>> tilefileNamesPerIP = null;
            BuildTilesIndex(ips, fileNames, listFTNs, out tilefileNamesPerIP);
            //log.Add(string.Format("完成切片索引更新。"));
            _mainui.ShowmessageLabel2("正在更新满幅度与云量信息……");
            UpdateAvailabilityCloudInfoOfTiles(tilefileNamesPerIP, listFTNs);      //暂时为全库信息更新，应该优化为针对当前导入瓦片 joki 170410
                ...
                foreach (var item in files)
                {
                    ...
                    if (item.Substring(item.LastIndexOf("\\") + 1).Substring(0, 2) == "CS")
                    {
                        if (item.Substring(item.LastIndexOf(".") + 1) == "txt")
                        {
                            ...
                            //MySqlBaseUtilities mySqlBase = new MySqlBaseUtilities(str);
                            IDbBaseUtilities mySqlBase = baseUtilities.GetSubDbUtilByCon(str);
                ...
                foreach (string tilesPath in tilesPaths)
                {
                    ...
                    foreach (FileInfo fi in fileinfos)
                    {
                        if (fi.Name.ToUpper().EndsWith(".TIF") || fi.Name.ToUpper().EndsWith(".JPG") || fi.Name.ToUpper().EndsWith(".PNG"))
                        {
                            ...
                            string newtilename = Path.GetFileName(desPath);
                            if (ip != "-1" && ip != "0")
                            {
                                ...
                                try
                                {
                                    ...
                                    tilefileNames1.Add(newtilename, ip);
                                }
                                catch
                                {
                                    continue;
        ...
        private void BuildTilesIndex(string[] ips, string[] filenames, List<string> listFTNs, out Dictionary<string, List<string>> tilefileNamesPerIP)
        {
            //建立切片索引参数
            tilefileNamesPerIP = new Dictionary<string, List<string>>();
            ...
            if (System.Text.RegularExpressions.Regex.IsMatch(CenterIP, pattern))
            {
                ...
                switch (Constant.DbStorage)
                {
                    case EnumDbStorage.MULTIPLE:
                        path = string.Format(@"\\{0}\{1}\{2}\", CenterIP, StorageBasePath.QRST_DB_Tile,
        ...
        private void UpdateAvailabilityCloudInfoOfTiles(Dictionary<string, List<string>> tilefileNamesPerIP, List<string> listFTNs)
        {
            List<Task> updateIndexTasks = new List<Task>();
            try
            {
                foreach (KeyValuePair<string, List<string>> kvp in tilefileNamesPerIP)
                {
                    ...
                    listName.Add(kvp.Key);
                    listName.AddRange(kvp.Value);
                    Task updateIndexTask = new Task(o =>
                    {
                        try
                        {
                            List<string> namelist = o as List<string>;
                            TServerSite site = TServerSiteManager.getSiteFromSiteIP(namelist.ElementAt(0));
                            namelist.RemoveAt(0);
                            site.TCPService.UpdateAvailabilityCloudInfoOfTiles(namelist);
                        }
                        catch (Exception ex)
                        {
                            System.Windows.Forms.MessageBox.Show(ex.Message);
                ...
                Task updateFailedIndexTask = new Task(p =>
                    {
                        try
                        {
                            List<string> files = p as List<string>;
                            string serverIP = TServerSiteManager.GetCenterSiteIP();
                            TServerSite site = TServerSiteManager.getSiteFromSiteIP(serverIP);
                            site.TCPService.UpdateAvailabilityCloudInfoOfTiles(files);
                        }
                        catch (Exception ex)
                        {
                            System.Windows.Forms.MessageBox.Show(ex.Message);
                ...
                updateFailedIndexTask.Start();
                updateIndexTasks.Add(updateFailedIndexTask);
            }
            catch (Exception ex)
            {
                System.Windows.Forms.MessageBox.Show(ex.Message);
        ...
        /*原 全局检索更新云量满幅度，因改为仅对当前导入数据进行计算，因此过时
         //SqliteWsImportUtils BaseUtil;
         //Dictionary<int, string> taskidip;
         //public DataSet UpdateAvailabilityCloudInfoOfTiles()
         //{
         //    BaseUtil = new SqliteWsImportUtils();
         //    taskidip = new Dictionary<int, string>();
         //    DataSet returnDs;
         ...
         //    {
         //        List<string> ipTilepath = new List<string>();
         //        string tilepath = BaseUtil.GetCommonSharePathBaseIP(IP);

         //        ipTilepath.Add(IP);

         //        ipTilepath.Add(tilepath);

         //        Task t = new Task(o => BaseUtil.UpdateAvailabilityCloudInfoOfTiles((List<string>)o), ipTilepath);
         //        t.Start();
         //        tasks.Add(t);
         //        taskidip.Add(t.Id, IP);
         //    }

         //    WaitingForTasks(tasks, 150000);

         //    returnDs = BaseUtil.MergeAllDataSet(BaseUtil.allDtCol);
        ...
        ///// <param name="outtime">超时时间 秒单位</param>
        //private void WaitingForTasks(List<Task> tasks, int outtime = 10)
        //{
        //    DateTime startdt = DateTime.Now;
        //    while (true)
        //    {
        //        if ((DateTime.Now - startdt).TotalSeconds > outtime)
        //        {
        //            break;
        //        }
        //        bool cmpl = true;
        ...
        //                if (tasks.Count - i < 4)
        //                {
        //                    Console.WriteLine(taskidip[t.Id]);
        //                }
        //            }
        //            else
        //            {
