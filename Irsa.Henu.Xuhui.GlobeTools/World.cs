...
namespace Qrst
{
    ...
    public class World : RenderableObject
    {
        ...
        /// <summary>
        /// 地球构造函数 <see cref= "T:Qrst.World"/> class.
        /// </summary>
            ...
            : base(name, position, orientation)
        {
            ...
            m_outerSphere.Init((float)equatorialRadius * 1.15f, 75, 75);
        }
        ...
        private void RenderSun(DrawArgs drawArgs)
        {
            ...
            {
                m_sunTexture = ImageHelper.LoadTexture(Path.Combine(((QrstAxGlobeControl)drawArgs.parentControl).DataDirectory, @"Space\sun.dds"));
        ...
        private void RenderStars(DrawArgs drawArgs, Qrst.Renderable.RenderableObject renderable)
        {
        ...
        public override void Render(DrawArgs drawArgs)
        {
            try
            {
                ...
                if (World.Settings.EnableSunShading)
                    RenderSun(drawArgs);
                //绘制地球表面
                Render(RenderableObjects, Qrst.Renderable.RenderPriority.TerrainMappedImages, drawArgs);

                //绘制大气
                Render(RenderableObjects, Qrst.Renderable.RenderPriority.AtmosphericImages, drawArgs);

                //绘制 线条
                Render(RenderableObjects, Qrst.Renderable.RenderPriority.LinePaths, drawArgs);

                //render Icons
                //绘制图标
                Render(RenderableObjects, Qrst.Renderable.RenderPriority.Icons, drawArgs);

                //render Custom
                //绘制自定义
                Render(RenderableObjects, Qrst.Renderable.RenderPriority.Custom, drawArgs);
        ...
        /// <param name="drawArgs"></param>
        private void Render(Qrst.Renderable.RenderableObject renderable, Qrst.Renderable.RenderPriority priority, DrawArgs drawArgs)
        {
            ...
            {
                //绘制Icon图层
                if (priority == Qrst.Renderable.RenderPriority.Icons && renderable is Icons)
                {
                    renderable.Render(drawArgs);
                }
                //绘制RenderableObjectList类型图层下的所有子图层
                else if (renderable is Qrst.Renderable.RenderableObjectList)
                {
                    Qrst.Renderable.RenderableObjectList rol = (Qrst.Renderable.RenderableObjectList)renderable;
                    for (int i = 0; i < rol.ChildObjects.Count; i++)
                    {
                        Render((Qrst.Renderable.RenderableObject)rol.ChildObjects[i], priority, drawArgs);
                    }
                }
                //绘制Qrst.Renderable.RenderPriority.SurfaceImages类型的图层
                else if (priority == Qrst.Renderable.RenderPriority.TerrainMappedImages)
                {
                    if (renderable.RenderPriority == Qrst.Renderable.RenderPriority.SurfaceImages || renderable.RenderPriority == Qrst.Renderable.RenderPriority.TerrainMappedImages)
                    {
                        renderable.Render(drawArgs);
                    }
                }
                //绘制Qrst.Renderable.RenderPriority.LinePaths和Qrst.Renderable.RenderPriority.AtmosphericImages类型的图层
    ...
    public class AtmosphericScatteringSphere
    {
        ...
        {
            Qrst.Camera.CameraBase camera = drawArgs.WorldCamera;
        ...
        void device_DeviceReset(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Stream skyFromSpaceStream = assembly.GetManifestResourceStream("Qrst.Shaders.SkyFromSpace.fx");
                ...
                Stream skyFromAtmosphereStream = assembly.GetManifestResourceStream("Qrst.Shaders.SkyFromAtmosphere.fx");
