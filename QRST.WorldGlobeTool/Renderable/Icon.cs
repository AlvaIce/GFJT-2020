...
namespace QRST.WorldGlobeTool.Renderable
{
    ...
    public class Icons : RenderableObjectList
    {
        public override Extension Extension
        {
            get
            {
                ...
                foreach (Icon icon in this.ChildObjects)
                {
        ...
        /// AwaysVisible名字的颜色
        /// </summary>
        static int awaysvisiblenameColor = Color.FromArgb(200, 220, 220, 220).ToArgb();
        /// <summary>
        /// 描述信息的文字颜色，三维影像模式下
        /// </summary>
        public int DescriptionColorAtImageMode = Color.White.ToArgb();
        /// <summary>
        /// 描述信息的文字颜色，三维地图模式下
        /// </summary>
        public int DescriptionColorAtMapMode = Color.White.ToArgb();
        /// <summary>
        /// 刷新时间
        /// </summary>
        ...
        protected virtual void Render(DrawArgs drawArgs, Icon icon, Vector3 projectedPoint)
        {
            ...
            if (iconTexture == null || isMouseOver || icon.NameAlwaysVisible || Icon.IconNameAllVisible)
            {
                // 绘制图标的名称
                if (icon.Name != null)
                {
                    ...
                    if (iconTexture == null)
                    {
                        ...
                        if (!is3DMapMode)
                            drawArgs.iconNameFont.DrawText(m_sprite, icon.Name, rect, DrawTextFormat.Center, DescriptionColorAtImageMode);
                        else
                            drawArgs.iconNameFont.DrawText(m_sprite, icon.Name, rect, DrawTextFormat.Center, DescriptionColorAtMapMode);
                    }
                    else
                    {
                        ...
                        if (Icon.IconNameAllVisible)
                        {
                            if (isMouseOver)
                            {
                                if (!is3DMapMode)
                                    drawArgs.iconNameFont.DrawText(m_sprite, icon.Name, rect, DrawTextFormat.WordBreak, DescriptionColorAtImageMode);
                                else
                                    drawArgs.iconNameFont.DrawText(m_sprite, icon.Name, rect, DrawTextFormat.WordBreak, DescriptionColorAtMapMode);
                            }
                            else
                            {
                                drawArgs.iconAwaysVisbleNameFont.DrawText(m_sprite, icon.Name, rect, DrawTextFormat.WordBreak, awaysvisiblenameColor);
                            }
                        }
                        else
                        {
    ...
    {
        public static bool IconNameAllVisible = true;
        public override Extension Extension
        {
            get
            {
        ...
        public override void Initialize(DrawArgs drawArgs)
        {
            ...
            //计算Icon的实际显示高度=夸大因子*Altitude+海拔高度
