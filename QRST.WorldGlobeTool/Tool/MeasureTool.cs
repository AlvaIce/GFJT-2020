...
//Shape support
using QRST.WorldGlobeTool.BaseTool;
...
namespace MeasureTool.Plugins
{
    ...
    /// </summary>
    public class MeasureTool : BaseWwTool
	{
		...
		public override void Load() 
		{
			layer = new MeasureToolLayer(
				this,
				ParentApplication.DrawArgs );

			layer.TexturePath = Path.Combine(PluginDirectory,"Plugins\\Measure");
			ParentApplication.CurrentWorld.RenderableObjects.Add(layer);

			menuItem = new MenuItem("Measure\tM");
			menuItem.Click += new EventHandler(menuItemClicked);
            ParentApplication.MenuBar.ToolsMenuButtons.Add( menuItem );

			// Subscribe events
			ParentApplication.MouseMove += new MouseEventHandler(layer.MouseMove);
			ParentApplication.MouseDown += new MouseEventHandler(layer.MouseDown);
			ParentApplication.MouseUp += new MouseEventHandler(layer.MouseUp);
			ParentApplication.KeyUp +=new KeyEventHandler(layer.KeyUp);
		}
		...
		public override void Unload() 
		{
			if(menuItem!=null)
			{
                ParentApplication.MenuBar.ToolsMenuButtons.Remove(menuItem);
				menuItem.Dispose();
				menuItem = null;
			}
			...
			ParentApplication.CurrentWorld.RenderableObjects.Remove(layer);
		}
	...
	public class MeasureToolLayer : QRST.WorldGlobeTool.Renderable.RenderableObject
	{
		...
		public MeasureToolLayer(MeasureTool srcPlugin, DrawArgs drawArgs) : base("Measure Tool")
		{
			...
			m_drawArgs = drawArgs;
            m_world = srcPlugin.ParentApplication.CurrentWorld;
		...
		/// </summary>
		protected void OnPropertiesClick(object sender, EventArgs e)
		{
		...
		bool CalculateRectPlacement(DrawArgs drawArgs)
		{
			...
			if (World.Settings.MeasureMode == MeasureMode.Multi)
			{
				...
				labelText = "Total Distance: " + ConvertUnits.GetDisplayAltitudeString(Distance);
			}
			else
			{
				//labelText = Distance>=10000 ?
                ...
                labelText = "Distance: " + ConvertUnits.GetDisplayAltitudeString(Distance);
            ...
            //为原来的LabelTextRect做个(4,1)的buffer,即外框
			Rectangle tsize = labelTextRect;
			const int xPad = 4;
			const int yPad = 1;
			tsize.Inflate( xPad, yPad );        //扩展x轴左右各拓展4单位,y轴上下各拓展1单位
            //然后整体平移回原来的起始位置
			labelTextRect.Offset(-tsize.Left, -tsize.Top);      //平移
			tsize.Offset(-tsize.Left, -tsize.Top);          //平移
		...
		int FindAnchorPoint()
		{
			...
			if(IsMeasureLinePointVisible(mid))
				return mid;
            //由于第一个点和最后一个点可能是和邻线共享点,因此作为锚点会有歧义.
            //不在,则取倒数第二个点
